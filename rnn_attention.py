...
class RNNModel(nn.Module):
    ...
    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers, hidden_size=128, dropout=0.5, tie_weights=False, attention=True,
        ...
        self.layernorm_init = nn.LayerNorm(128)
    ...
    def forward(self, input, init=None):
        ...
        # print("input size:",init.shape())
        # init_hidden, init_cell = init.view(-1, 2, 2, 128).permute(1, 2, 0, 3).contiguous()
        # print("input size:",init_hidden.shape(), init_cell.shape)
        # emb = self.drop(self.encoder(input))
        # emb = self.drop(input)\
        emb = input
        # print("emb size:",emb.size(), init_hidden.size, init_cell.size)
    ...
    def forward(self, inputs, attention_width=3):
        ...
        for i in range(inputs.size(0)):
            if(i<attention_width):
                ...
                vec = torch.index_select(inputs, 0, selector).cuda()
