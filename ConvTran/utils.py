...
class dataset_class(Dataset):
    ...
    def __init__(self, data, label, clip_length):
    ...
    def __getitem__(self, ind):
        ...
        lengths = x.shape[0]
        cp = random.randint(0, lengths - 1 - self.clip_length) if self.clip_length < lengths else 0
        cp = 0
        # print('fffffffffffffffffffffffffff',x.shape)
        x = x[1:]
        x = x[::2 , cp: cp + self.clip_length]
        
        # print('wwwwwwwwwwwwwwwwwww',x.shape)
        y = self.labels[ind]  # (num_labels,) array

        data = torch.tensor(x)
        label = torch.tensor(y)
        # print(data.shape, label, ind)
