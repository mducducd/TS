...
# -------------------------------------------- Input and Output --------------------------------------------------------
parser.add_argument('--data_path', default='/media/duke/data/TimeSeres_pool/ai4pain', choices={'Dataset/UEA/', 'Dataset/Segmentation/'},
                    help='Data path')
...
parser.add_argument('--val_interval', type=int, default=10, help='Evaluate on validation every XX epochs. Must be >= 1')
...
parser.add_argument('--clip_length', default=64, type=int, help='Seed used for splitting sets')
...
if __name__ == '__main__':
    ...
    for problem in os.listdir(config['data_path']):  # for loop on the all datasets in "data_dir" directory
        ...
        train_dataset = dataset_class(Data['train_data'], Data['train_label'], config['clip_length'])
        val_dataset = dataset_class(Data['val_data'], Data['val_label'], config['clip_length'])
        # test_dataset = dataset_class(Data['test_data'], Data['test_label'], config['clip_length'])
        ...
        print('dddddddddddddddddddddddddddddd', config['num_labels'], config['Data_shape'])
        ...
        best_test_evaluator = SupervisedTrainer(best_model, val_loader, device, config['loss_module'],
                                                print_interval=config['print_interval'], console=config['console'],
                                                print_conf_mat=True)
        ...
        print(print_str)
        best_aggr_metrics_val, all_metrics = val_evaluator.evaluate(keep_all=True)
        print_str = 'Best Model Val Summary: '
