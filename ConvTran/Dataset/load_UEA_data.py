...
def load(config):
    # Build data
    config['data_dir'] = '/media/duke/data/TimeSeres_pool/'
    Data = {}
    # problem = config['data_dir'].split('/')[-1]
    problem = 'ai4pain'
    # if os.path.exists(config['data_dir'] + '/' + problem + '.npy'):
    if os.path.exists('/media/duke/data/TimeSeres_pool/ai4pain.npy'):
...
def process_ts_data(x, max_len, vary_len: str = "suffix-noise", normalise: bool = False):
    ...
    for i in range(num_dim):
        for j in range(num_instances):
            ...
            cp = random.randint(0, lengths - 1 - clip_length) if clip_length < lengths else 0
            # output = output[j, i, cp: cp + clip_length]
            output[j, i, cp: cp + clip_length] = x[columns[i]][j].values
